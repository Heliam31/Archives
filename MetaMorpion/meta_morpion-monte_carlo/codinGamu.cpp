...
int main()
{
    ...
    // - on a 1sec pour faire du montecarlo -
    int cpt = 0;
    
    do{
        
        MonteCarloTreeSearch(root, 0);
        cpt++;
    ...
    cerr << "nbr mcts pre-game: " << cpt << endl;
    ...
    while (1) {
        ...
        // - On execute ce code qu'on on est pas de le cas "premier tour et on commence" -
        if(opponent_col != -1 || opponent_row != -1){
            ...
            while(i < root->getChildArray().size() && !found){
                ...
                if(root->getChildArray()[i]->getState().getBoard().getLastM().getX() == morpionDesigneX && root->getChildArray()[i]->getState().getBoard().getLastM().getY() == morpionDesigneY){
                    ...
                    if(root->getChildArray()[i]->getState().getBoard().getLastP().getX() == caseDesigneX && root->getChildArray()[i]->getState().getBoard().getLastP().getY() == caseDesigneY && root->getChildArray()[i]->getState().getBoard().getContainer().getValueXY(morpionDesigneX, morpionDesigneY).getContainer().getValueXY(caseDesigneX, caseDesigneY) == 2){
            ...
            do{
                ...
                elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(end - begin);
            }while(elapsed.count() < 110);    //enlever le commentaire si timeout
        ...
        // - TOUR 1 si on commence: -
        }else{
            playerNo=1;
            root->getState().setPlayerNo(1);

            // - On joue en fonction du meilleur noeud fils calcule avec les multiples MCTS pre-game (avant boucle de jeu) -
            root = root->getChildWithMaxScore();
...
/*
    cerr << endl << "liste child" << endl;
    for(int i = 0; i < root->getChildArray().size(); i++){
        cerr << root->getChildArray()[i]->getState().getWinScore() << "/" << root->getChildArray()[i]->getState().getVisitCount() << endl;
    }

    cerr << endl << "child choisi: " << root->getState().getWinScore() << "/" << root->getState().getVisitCount() << endl;
*/
