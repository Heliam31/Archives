...
FROM LIVRE 
WHERE NOT CATEGORIE = 'MEDECINE' AND NOT CATEGORIE = 'SCIENCES' AND NOT CATEGORIE = 'LOISIR' ORDER BY CATEGORIE;
...
SELECT NUM_EX, D_EMPRUNT FROM EMPRUNT WHERE NUM_AB = (SELECT NUM_AB FROM ABONNE WHERE NOM='DUPONT' AND PRENOM='JEAN') ORDER BY D_EMPRUNT;
...
SELECT CATEGORIE 
FROM LIVRE  
WHERE CATEGORIE NOT IN (SELECT CATEGORIE FROM LIVRE INNER JOIN EXEMPLAIRE USING(ISBN) FULL JOIN EMPRUNT ON EXEMPLAIRE.NUMERO = EMPRUNT.NUM_EX) ;
...
SELECT COUNT(*) FROM ABONNE JOIN EMPRUNT ON ABONNE.NUM_AB = EMPRUNT.NUM_AB WHERE NOM = 'RENARD' AND PRENOM='ALBERT' AND D_RET_REEL IS NULL;
...
SELECT CATEGORIE FROM EXEMPLAIRE JOIN LIVRE ON EXEMPLAIRE.ISBN = LIVRE.ISBN WHERE PRIX=(SELECT MAX(PRIX) FROM EXEMPLAIRE);
...
SELECT NUMERO 
FROM EXEMPLAIRE JOIN EMPRUNT ON EXEMPLAIRE.NUMERO = EMPRUNT.NUM_EX 
...
SELECT * FROM MOT_CLEF FULL JOIN CARACTERISE ON MOT_CLEF.MOT = CARACTERISE.MOT WHERE ISBN IS NULL;
...
SELECT  NUM_AB, NOM, CATEGORIE, COUNT(*)
FROM ABONNE JOIN EMPRUNT USING (NUM_AB) JOIN EXEMPLAIRE ON EMPRUNT.NUM_EX=EXEMPLAIRE.NUMERO JOIN LIVRE USING (ISBN)
...
SELECT ISBN, TITRE,  AVG(PRIX) FROM EXEMPLAIRE JOIN LIVRE USING(ISBN) GROUP BY ISBN, TITRE HAVING COUNT(*) > 2;
...
SELECT NUM_AB, NOM
FROM ABONNE JOIN EMPRUNT USING(NUM_AB) GROUP BY NUM_AB, NOM 
...
FROM LIVRE  JOIN EXEMPLAIRE EX USING(ISBN) 
WHERE EX.CODE_PRET = 'EXCLU' AND ISBN IN (SELECT e2.ISBN FROM EXEMPLAIRE e2 JOIN EMPRUNT ON NUM_EX = e2.NUMERO HAVING COUNT(*)>2 GROUP BY e2.ISBN)
GROUP BY ISBN, TITRE;	
...
SELECT NUM_AB, NOM 
FROM ABONNE ab1 
WHERE EXISTS(SELECT NOM FROM ABONNE ab2 WHERE ab1.NOM = ab2.NOM AND NOT (ab1.NUM_AB = ab2.NUM_AB) );
...
SELECT CATEGORIE 
FROM LIVRE JOIN EXEMPLAIRE EX USING(ISBN) JOIN EMPRUNT EM ON EX.NUMERO = EM.NUM_EX
GROUP BY CATEGORIE
...
FROM livre L 
WHERE NOT EXISTS(SELECT E.isbn FROM exemplaire E WHERE E.isbn = L.isbn AND E.prix < 30)
	AND L.isbn IN (SELECT DISTINCT isbn FROM exemplaire) ORDER BY L.isbn;
...
FROM livre L
WHERE NOT EXISTS(SELECT E.isbn FROM exemplaire E WHERE E.isbn = L.isbn AND E.prix > 30 
		AND NOT EXISTS(SELECT num_ex FROM emprunt WHERE E.numero = num_ex GROUP BY num_ex HAVING COUNT(*)>=3) );
/*
Résultat attendu :SELECT isbn, titre FROM livre L WHERE L.isbn NOT EXISTS(SELECT E.isbn FROM exemplaire E WHERE E.isbn = L.isbn AND prix < 30);
ISBN		TITRE
--------------- --------------------------------------------------
...
SELECT l.isbn FROM livre l JOIN caracterise ON l.isbn = caracterise.isbn 
WHERE NOT EXISTS 
	(SELECT mot FROM caracterise C1 WHERE isbn ='0_8_7707_2' AND NOT EXISTS 
		(SELECT * FROM caracterise C2 WHERE C1.mot = C2.mot AND C2.isbn = l.isbn))
GROUP BY l.isbn
